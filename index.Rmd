...
Energy balances provide a lump-sum of the energy needs in the canton but for the planning and implementation of measures and policies a more granular spatial and temporal desegregation would be advantageous. For example, the emissions from liquid fuels could be made spatial for the case of transport by tapping into activity data from [comercial providors](https://www.tomtom.com/) (see Map 1) or data available in other ministries such as public transport usage, number of resisted vehicles and type, etc. This would allow to identify critical areas of intervention - see Map 1.
...
Correlating household gas consumption with seasonal-dependent variables (see Figure 2) woulds to estimate the influence of long-term climate change in energy demand cof the canton Sarajevo, and by extent its effect on GHG emissions. In the example provided, the highlighted dependency between gas consumption in households and temperature could be easily integrated in regional climate projection from the [IPCC](https://interactive-atlas.ipcc.ch/) in order to evaluate long-term needs for gas supply.  
...
Finally, an adequate leverage of the existing energy balances would allow the canton to initiate the process of investigating how infrastructure, regulatory and economic measures would translate to energy and GHG savings. For example, such could be done by looking to previous studies investigating the deployment of measures and measuring the subsequent effects (see Table 2). For the example provided its investigated the range of energy reductions in households brought about by the introduction of smart-meters. Savings in energy are then easily converted to GHG reductions.
